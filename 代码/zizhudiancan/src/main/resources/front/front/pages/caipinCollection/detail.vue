   	 ...
   	 <view style="margin-top: 10rpx;"></view>
					...
					<view class="detail-list-item"
						...
						<view class="text" :style='{"color":"#06A9FF","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.caipinCollectionValue}}</view>
					</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('caipinCollection', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.caipinCollectionPhoto ? this.detail.caipinCollectionPhoto.split(",") : [];
			},
